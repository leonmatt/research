...
from data_model import SupportPayload
from maintenance import test_maintenance
from maintenance import get_help
...
async def predict(payload:MaintenancePayload):
    ...
    maintenance_result = test_maintenance(payload.hydraulic_pressure)
    return {"msg": "Completed Analysis", "Maintenance Status": maintenance_result}

@app.post("/supportbot")
async def supportbot(payload:SupportPayload):

    help_result = get_help(payload.input_string)
    return {"message": help_result}
